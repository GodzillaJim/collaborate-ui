...
const TaskList = () => {
    ...
    const filteredTasks = React.useMemo(()=> {
        ...
        return anotherTemp.filter((task)=> task.name.toLowerCase().includes(search.toLowerCase()))
    }, [tasks, search])
    ...
    return <div className={"container-fluid"}>
        <div className={"row"}>
            <div className={"col"}>
                { loading && <Loading/> }
                { error && <ErrorMessage message={error} retryAction={handleGetTasks} /> }
                { tasks && <div className={"container-fluid"}>
                    ...
                    <Row xs={1} sm={2} md={3} className={"g-3"}>
                        { filteredTasks.map((task)=> {
                            return <Col className={"my-1"} key={`key-${v4()}`}>
                                <TaskCard {...task}/>
                            </Col>
                        }) }
                    </Row>
